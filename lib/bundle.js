...
class Tile {
  ...
  [0,12.02,21.12,29.24,36.92,44.23,51.18,57.46,63.48,69.40,73.72,77.70,80.58,
    ...
    for (let i = 1; i < letterFrequencies[1].length-1; i++) {
...
class View {
  ...
  constructor($el) {
    ...
    // need to increment the tiles across screen at diff rate than checking letters
    // does there need to be a interval for the letter submission?
    window.setInterval(
  ...
  handleSubmit(e) {
    ...
    if (true)
    {
      ...
      this.render();
      this.string = "";
      // clear word from the input field if needed
    }
  }
...
class Board {
  ...
  // NOTE: make sure to clear the left-most first!
  clearWord(word) {
    let tempWord = word;
