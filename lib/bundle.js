...
class View {
  ...
  descendingInterval() {
    ...
    console.log(this.stepMillis()); // firebug or chrome log
    clearInterval(this.interval); // stop the setInterval()
  ...
  stepMillis() {
    switch (true) {
      case (!this.game):
        return 7000;
      case (this.game.score < 10):
        return 7000;
      case (this.game.score < 20):
        return 5000;
      case (this.game.score < 50):
        return 4000;
      case (this.game.score < 70):
        return 3000;
      case (this.game.score >= 70):
        return 3000;
    }
  ...
  handleEnter(e) {
    if (e.keyCode == 13) {
      if (document.getElementById("intro")) {
        ...
        if (document.getElementById("game-over") === null) {
          e.preventDefault();
          $( "#word-form" ).submit();
        } else {
          e.preventDefault();
  ...
  setupBoard() {
    ...
    // adding the bar below the tiles
    const $ledge = $("<div>");
    $ledge.attr('id','ledge');
    $(this.$el).append($ledge);

    // introductory box
    const $intro = $("<div><h1>INSTRUCTIONS</h1><p>Create words out of the letters on the screen. Use the letters before they hit the red bar. Words are 3 letters minimum, and longer words earn you more points. Good luck!</p></div>");
    $intro.attr('id','intro');
